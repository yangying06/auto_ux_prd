...
export interface CocosMotionIntegrationMode {
  id: string
  label: string
  keywords: RegExp
...
export interface CocosMotionRule {
  type: string
  weight: number
  pattern: RegExp
...
export const COCOS_MOTION_INTEGRATION_MODES: CocosMotionIntegrationMode[] = [
  {
    id: 'tween-transform',
    label: 'Cocos Tween 变换',
    keywords: /tween|Tween|位移|移动|飞入|飞出|缩放|弹出|收起|旋转|闪烁|缓动|贝塞尔|曲线|position|scale|easing/i,
    designerQuestions: [
      '这个动效需要用程序 Tween 控制位置、缩放或旋转吗？请确认起点、终点、时长和缓动。',
  ...
  {
    id: 'animation-clip',
    label: 'AnimationClip/cc.Animation',
    keywords: /AnimationClip|cc\.Animation|\.anim|动画剪辑|帧动画|playOnLoad|Animation\b/i,
    designerQuestions: [
      '这段动效是否已有 Cocos AnimationClip？请给出 clip 名称、是否自动播放和循环方式。',
      'clip 播完后是否需要派发事件、切状态或继续播放下一段程序 Tween？',
    ],
  ...
  },
  {
    ...
    designerQuestions: [
      '这个表现由哪个 prefab 承载？实例化到哪个 layer，是否需要对象池复用？',
      'prefab 的入场、停留、关闭和销毁/回收时机分别是什么？',
    ],
...
export const COCOS_MOTION_RULES: CocosMotionRule[] = [
  {
    type: '结果/奖励表现',
    weight: 28,
    pattern: /中奖|jackpot|bigwin|bonus|free|抽中|抽奖|奖励|大奖|命中奖励|获得奖励|掉落|开箱|结算奖励|权益解锁|解锁奖励/i,
    integrationModes: ['Spine/Skeleton', 'Prefab 特效/弹窗', '音效联动', 'Cocos Tween 变换'],
    sequence: [
    ...
    questions: [
      '这个结果由哪个字段或条件决定？不同等级是否播放不同 Spine/Prefab/音效？',
  ...
  },
  {
    ...
    integrationModes: ['Cocos Tween 变换', 'UIOpacity 淡入淡出', 'ParticleSystem 粒子', '序列帧', '音效联动'],
    sequence: [
      ...
      { title: '最终刷新', detail: '飞入结束后刷新最终资产值，并确认是否回收飞行 prefab。', layer: '数据展示层' },
    ],
    questions: [
  ...
  },
  {
    ...
    integrationModes: ['Cocos Tween 变换', 'Spine/Skeleton', 'ParticleSystem 粒子', '音效联动'],
    sequence: [
  ...
  },
  {
    ...
    integrationModes: ['Spine/Skeleton', 'Cocos Tween 变换', 'Prefab 特效/弹窗', '音效联动'],
    sequence: [
  ...
  },
  {
    ...
    integrationModes: ['Prefab 特效/弹窗', 'AnimationClip/cc.Animation', 'UIOpacity 淡入淡出', '音效联动'],
    sequence: [
  ...
  },
  {
    ...
    integrationModes: ['Cocos Tween 变换', 'Spine/Skeleton', 'ParticleSystem 粒子', 'AnimationClip/cc.Animation', '音效联动'],
    sequence: [
  ...
  },
  {
    ...
    integrationModes: ['Prefab 特效/弹窗', 'UIOpacity 淡入淡出', '音效联动'],
    sequence: [
