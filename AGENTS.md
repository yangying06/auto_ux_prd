...
- **AI后端**: 继续使用Anthropic Codex API（通过本地Express代理）
...
## Key Dependencies
- `@anthropic-ai/sdk` ^0.97.1 - Codex API client; used only in `server/index.ts`; requires `ANTHROPIC_API_KEY` env var
...
- Optional: `ANTHROPIC_BASE_URL`, `CLAUDE_MODEL` (defaults to `Codex-sonnet-4-6`), `LOCAL_PROXY_PORT` (defaults to `8787`), `COCOS_RAG_SSE_URL`, `COCOS_RAG_PROXY_SCRIPT`, `APPDATA`
...
- Purpose: Bridges the frontend to the Anthropic Codex API and Cocos RAG MCP service; handles all secret management
- Location: `server/index.ts`
- Contains: Express routes, Codex agentic loop, state normalization, MCP subprocess management
...
- Filled by: AI responses via `state_patch` JSON embedded in Codex replies
...
- `safeParseClaudeJson()` has a two-candidate fallback parser so malformed Codex JSON doesn't hard-crash the server
